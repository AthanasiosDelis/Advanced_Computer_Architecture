...
def get_joule_from_power_file(output_file):
    joule = -999
...
for dirname in sys.argv[2:]:
    ...
    joule = get_joule_from_power_file(power_file)
    time  = (get_time_from_output_file(output_file)) /1000000000.0 # convert ns to seconds
    print(joule,time)
    edp = time * joule
...
plt.savefig(sys.argv[1]+bench+'-'+input_size+'.edp.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.clf
ax.clear()
