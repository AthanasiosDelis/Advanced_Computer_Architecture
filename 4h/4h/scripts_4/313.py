...
# A path needs changing, check the get_runtime() function
...
def get_runtime(sync, nthreads, grain):
    path = "/home/thanasis-linux/ex4-helpcode/output/" + str(grain) + "/" + sync + "_" + str(nthreads) + "/sim.out"
    fp = open(path, "r")
    line = fp.readline()
    while line:
        if "Time (ns)" in line:
...
for dirname in sys.argv[2:]:
    ...
    runtime = get_runtime(sync, nthreads, grain)
    edp = (energy * runtime) / (pow(10, 9))
    results_tuples.append((sync, nthreads, edp))
...
plt.savefig("313_" + str(grain) + '.edp.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
